...
import {CallListener} from '../CallListener.js'
...
import {CallServiceDelegate} from './CallServiceDelegate.js'
...
export class CallServiceWorker extends CallService {
    ...
    answerCall() {
        ...
        let constraints = self._generateMediaConstraints();
        if (self.checkAndStart()) {
            // 回调正在处理
            self.delegate.didProgress(self, self.target);
            self.pc.setRemoteDescription(new RTCSessionDescription({type: "offer", sdp: self.sdpCache}), function () {
                self.canAddIceCandidate = true;
                self.drainCandidateQueue();
                window.utils.getUserMedia(self.videoEnabled ? constraints : false, true, (err, stream) => {
                    if (err) {
                        self.isStarted = false;
                        self.handleUserMediaError(err);
                    } else {
                        self.handleUserMedia(stream);
    ...
    makeCall(callee, video, callback) {
        ...
        //禁止呼叫自己
        if (callee == this.engine.session.name) {
            Logger.d("SignalingWorker", "You can't call yourself");
            return false;
        }
        this.flagTerminated = false;
    ...
    _generateMediaConstraints() {
        var mW = 320;
        var mH = 240;
        if (null != this.maxVideoSize) {
            if (this.maxVideoSize.width !== undefined)
                mW = this.maxVideoSize.width;
            if (this.maxVideoSize.height !== undefined)
                mH = this.maxVideoSize.height;
        }
    ...
    invite(callee, video) {
        ...
        let constraints = this._generateMediaConstraints();
        ...
        if (this.checkAndStart()) {
            ...
            window.utils.getUserMedia(this.videoEnabled ? constraints : false, true, (err, stream) => {
                if (err) {
                    this.isStarted = false;
    ...
    onDialogue(action, dialect) {
        ...
        else if (action == CubeConst.ActionJoinConferenceAck) {
            Logger.d('SignalingWorker#onDialogue', CubeConst.ActionJoinConferenceAck);
    ...
    handleUserMediaError(error) {
        ...
        window.utils.getUserMedia(false, true, (err, stream) => {
            if (err) {
                this.delegate.didFailed(this, this.target, CubeStateCode.MicphoneOpenFailed);
            } else {
                window.utils.closeMediaStream(stream);
    ...
    _processInvite(dialect) {
        ...
        if (this.state == SignalingState.None || this.state == SignalingState.Invite) {
            ...
            this.targetData = data.callerData;
            ...
            /*var mW = 320;
             ...
             if (self.autoAnswer) {
             setTimeout(function() { self.answerCall(); }, 60);
             }
             },
             function(e) {
    ...
    _processReverseCall(dialect) {
        ...
        //更新呼叫方向
        this.engine.session.direction = CallDirection.Incoming;
        //更新状态
        this.engine.session.state = SignalingState.Invite;
    ...
    queryConference(groupName, callback, errorCallback) {
        ...
        dialect.appendParam("param", {"token": _MSTR_TOKEN, "conferenceId": groupName});
        return nucleus.talkService.talk(CELLET.Signaling, dialect);
    }
    ...
    _processQueryAllConference(dialect) {
        ...
        if (state.code == 200) {
            ...
            confService.delegate.onConferenceFailed(new CubeError(state.code, 'Query Conference Failed!'))
        }
    }

    _processJoinConference(dialect) {
        let state = dialect.getParam("state");
        let data = dialect.getParam("data");
        let confService = this.engine.getConferenceService();
        let conferenceId = state.code != 200 ? (data ? data.conferenceId : dialect.getParam("conferenceId")) : '';
        if (state.code == 200) {
            confService.applyJoinCallback()
        }
        else {
            confService.delegate.onConferenceFailed(new CubeError(state.code, 'Join Failed!'), conferenceId);
            this.delegate.didEnd(this, conferenceId, "cancel");
        }
    }
